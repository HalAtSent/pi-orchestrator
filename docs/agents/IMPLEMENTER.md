...
- opportunistic refactors outside the need of the approved fix
- silent architecture changes
- knowingly partial fixes
...
- Make the slightly broader change when the smaller patch would obviously rot.
...
- `why_this_is_sufficient`: why the change fully solves the approved problem and
  why a narrower patch would not be durable when that tradeoff exists
- `what_was_verified`: the tests, checks, or inspections actually performed
